...
std::string
BroField::tableType() const {
    return tableType_;
}

void
BroField::setType(std::string type) {
    type_      = type;
    tableType_ = "TEXT";
...
void
BroHeader::readFields(std::string &input) {
    int  pos    = 0;
...
void
BroHeader::readTypes(std::string &input) {
...
bool
BroHeader::read(fs::path &logPath) {
...
void
BroHeader::readHeader(std::string &line) {
...
void
BroHeader::parse(std::string &line, QueryData &results) {
